...
import { CreateReservoirType, UpadateReservoirType } from "@/types/types";
...
export const createReservoir = async (reservoir: CreateReservoirType) => {
  try {
    const response = await fetch(`${apiUrl}/reservoirs/`, {
      method: "POST",
      headers: {
...
export const updateReservoir = async (
  reservoirId: number | undefined,
  updatedReservoir: UpadateReservoirType
) => {
  try {
    const response = await fetch(`${apiUrl}/reservoirs/${reservoirId}`, {
      method: "PUT",
